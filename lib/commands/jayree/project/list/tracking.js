...
class SourceTrackingList extends SfCommand {
    async run() {
        ...
        const maxrevpath = await getCurrentStateFolderFilePath(this.project.getPath(), join('orgs', org.getOrgId(), 'maxRevision.json'), false);
        let maxrevfile;
        try {
        ...
        this.styledHeader(chalk.blue(`SourceMember revision counter numbers list for: ${org.getUsername()}/${org.getOrgId()}`));
...
SourceTrackingList.deprecateAliases = true;
SourceTrackingList.aliases = ['jayree:source:tracking:list'];
export default SourceTrackingList;
